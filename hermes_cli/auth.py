...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "claude-cli":
        ...
        # The claude-cli client appends the inference flags (`-p`, etc.); no
        # special launch args are needed here.
        raw_args = os.getenv("HERMES_CLAUDE_CLI_ARGS", "").strip()
        args = shlex.split(raw_args) if raw_args else []
        resolved_command = shutil.which(command) if command else None
        if not resolved_command:
            raise AuthError(
                ...
                "Install the Claude CLI or set HERMES_CLAUDE_CLI_COMMAND/CLAUDE_CLI_PATH.",
                provider=provider_id,
